...
from src.service.vault.texts import UNPURGE_EMPTY, UNPURGE_FAIL, UNPURGE_INLINE_HINT, UNPURGE_INLINE_TITLE, UNPURGE_NONE, UNPURGE_NO_RIGHTS, txt_shop_page, txt_unpurge_header, txt_unpurge_ok
...
async def cmd_unpurge(message: Message, user: User, users: UserRepository):
    if user.role < Role.OWNER:
        await send_msg(message, UNPURGE_NO_RIGHTS, only_caller=True)
        return
    ...
    if message.chat.type == "private":
        ...
        await send_msg(message, UNPURGE_INLINE_HINT, reply_markup=kb_unpurge_switch(), only_caller=True)
...
async def clbck_unpurge_page(callback: CallbackQuery, bot: Bot, user: User, users: UserRepository):
    if user.role < Role.OWNER:
        await callback.answer(UNPURGE_NO_RIGHTS, show_alert=True)
        return
...
async def clbck_unpurge_noop(callback: CallbackQuery, user: User):
    ...
    await callback.answer(txt_shop_page(parts[1], parts[2]))
